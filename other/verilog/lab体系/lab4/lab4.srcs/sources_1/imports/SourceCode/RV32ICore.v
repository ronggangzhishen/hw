...
module RV32ICore(
    ...
    output wire debug_miss, debug_req_signal,
    output wire debug_branch, debug_predict_success, debug_predict_fail
    );
    ...
    assign debug_req_signal = req_signal;
       
    //for predict
    wire br_predict_IF, br_predict_ID, br_predict_EX; //br_predict信号
    wire [31:0] br_target_predict;    //IF段目标
    ...
    // predictor debuger
    assign debug_branch = br_type_EX == 3'b0 ? 0 : 1;
    assign debug_predict_success = ((br && br_predict_EX) || 
                                    (debug_branch && !br && !br_predict_EX));
    assign debug_predict_fail = ((br && !br_predict_EX) || 
                                    (debug_branch && !br && br_predict_EX));
    ...
    NPC_Generator NPC_Generator1(
        ...
        .jalr_target(ALU_out),
        .br_target(br_target),
        .br_target_predict(br_target_predict),  //p
        .PC_EX(PC_EX),                          //p
        .br_predict_IF(br_predict_IF),          //p
        .br_predict_EX(br_predict_EX),          //p
        .jal(jal),
        .jalr(jalr_EX),
        .br(br),
        .NPC(NPC)
    ...
    PC_ID PC_ID1(
        ...
        .flushD(flushD),
        .PC_IF(PC_4),
        .br_predict_IF(br_predict_IF),      //add
        .br_predict_ID(br_predict_ID),      //add
        .PC_ID(PC_ID)
    );
    ...
    branchpredictor BranchPredictor1(
        ...
        //EX段输入, 更新表
        .PC_EX(PC_EX),
        .br(br),                //根据br情况更新BTB, BHT表
        .br_target(br_target),
        //IF段输入, 得预测
        .PC_IF(PC_4),          //用于查找存在的分支指令
        //IF段输出
        .br_predict_IF(br_predict_IF),          //传出的信号
        .br_target_predict(br_target_predict)   //
    );
    ...
    Ctrl_EX Ctrl_EX1(
        ...
        .br_predict_ID(br_predict_ID),  //add
        .br_predict_EX(br_predict_EX),  //add
        .jalr_EX(jalr_EX),
