...
module cache #(
    ...
    input  clk, rst,
    output miss, req_signal,              // 对CPU发出的miss信号
...
//**
assign req_signal = wr_req | rd_req;
reg cache_hit = 1'b0;
reg [31:0]way_addr = 32'b0;
reg [31:0] history[SET_SIZE][WAY_CNT];
always @ (*) begin              // 判断 输入的address 是否在 cache 中命中              
    ...
    if (cache_hit == 1'b0) begin
        ...
        begin
            if(history[set_addr][way_addr] < history[set_addr][i]) 
            begin
                way_addr = i;
            end
        end
...
module cache #(
    ...
    parameter  WAY_CNT       = 1  // 组相连度，决定了每组中有多少路line, 为1时就为直接映射
)(
    input  clk, rst,
...
reg cache_hit = 1'b0;
// way_idx，当命中时，这个变量为该组中某个line的下标，当未命中时，这个变量为要换出的line的下标
reg [31:0]way_idx;
// 每个line的历史信息，对于FIFO是其被换入的时间，对于LRU是其上一次被引用到现在的时间
reg [31:0]history[SET_SIZE][WAY_CNT]; 



// 对输入的每个rd或wr，都只会生成一个周期的高电平信号req_signal
// req_signal只会一个周期有效, 避免在一次读写中多次改变历史信息记录
reg rec_signal; //temp
...
begin
    ...
    begin
        if (rd_req | wr_req) 
        begin
            if (rec_signal == 1'b0 && req_signal == 1'b0) 
            begin
                rec_signal <= 1'b1;
                req_signal <= 1'b1;
            end
            else if (rec_signal == 1'b1 && req_signal == 1'b1)
            begin
                rec_signal <= 1'b1;
                req_signal <= 1'b0;
            end
...
begin
    ...
    begin
        if (LRU == 1'b1) 
        begin // 若使用 LRU 策略
        // history[set_addr] 中的值记录的是某set上次被引用距现在的次数
            if (req_signal) 
            begin 
                for(integer i = 0; i < WAY_CNT; ++i) 
                begin
...
always @ (*) begin              
    ...
    if (cache_hit == 1'b0) 
    begin
        //若cache未命中，way_idx是history中选出的最大的下标，也就是LRU或者FIFO状态机选出的要替换的下标
        for (integer i = 0; i < WAY_CNT; ++i)
        begin
...
wire mem_rd_req = (cache_stat == SWAP_IN );
wire mem_wr_req = (cache_stat == SWAP_OUT);
wire [   MEM_ADDR_LEN-1 :0] mem_addr = mem_rd_req ? mem_rd_addr : 
                                                    ( mem_wr_req ? mem_wr_addr : 0);
